...
namespace nORM
{
    namespace SQL
    {
        internal sealed class PostgreSQLQueryFactory : IQueryFactory
        {
            #region Singleton
            ...
            public SelectQuery Select(string source, string[] fields, string SourceAlias) => new PostgreSQLSelectQuery(source, fields, SourceAlias);
...
#warning add debug output
        ...
        internal sealed class PostgreSQLSelectQuery : SelectQuery
        {
            ...
            {
                throw new NotImplementedException("COUNT_BIG is not available in PostgreSQL");
#warning think!!!
                var clone = Clone() as PostgreSQLSelectQuery;
...
#warning interference with TOP statement
                ...
                return new TSQLSelectQuery(clone.ToString(), new string[] { "CAST(COUNT(*) AS BOOLEAN)" }, "T");
            }
        }
    }
}